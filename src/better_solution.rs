    ...
    impl BetterSolution {
        fn calculate_left_highest_points(&mut self) {
            ...
            for &item in self.vec.iter() {
                ...
                self.left_vec.push(current_highest_point);
            }
        }
        ...
        fn calculate_right_highest_points(&mut self) {
            ...
            for i in (0..self.vec.len()).rev() {
                ...
                self.right_vec.push(current_highest_point);
            }
        ...
        pub fn compute(&mut self) -> i32 {
            ...
            for (index, &current_item) in self.vec.iter().enumerate() {
                total_stored_rain += self.get_stored_rain(
                    self.left_vec[index],
                    self.right_vec[self.right_vec.len() - 1 - index],
                    current_item,
                );
            }
